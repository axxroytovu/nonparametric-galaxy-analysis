...
def SingleGalaxy(GalaxyNumber, DataDirectory):
    ...
    def SCALE(arr):
        return np.log((np.exp(1)-1)*(arr-Vmin)/(Vmax-Vmin) + 1)
    CImage = SCALE(CImage)
    ...
    Data = [["Filename", "Petrosian Radius", "Gini Coefficient", "M20",
             "Asymmetry 90", "Asymmetry 180", "Concentration", "Clumpiness",
             "Multimode", "Intensity", "Deviation"]]
    Graph = True
    for f in filenames:
        hdulist = fits.open(DataDirectory+f)
        # print hdulist.info()
        AllDat = ma.masked_array(np.array(hdulist[0].data),
                                 segarray != GalaxyNumber)
        # print AllDat[0:10]
        rp = PetrosianRad(AllDat)
        MaxCoord, I = Intensity(AllDat, graph=Graph)
        A1, A2 = Asymmetry(AllDat, rp, graph=Graph)
        Data.append([f, str(rp), str(gini(AllDat, graph=Graph)),
                     str(M20(AllDat, graph=Graph)), str(A1), str(A2),
                     str(Concentration(AllDat, rp, graph=Graph)),
                     str(Clumpiness(AllDat, rp, graph=Graph)),
                     str(Multimode(AllDat, graph=Graph)), str(I),
                     str(Deviation(AllDat, MaxCoord, graph=Graph))])
        hdulist.close()
        Graph = False
...
def BuildDatabase(subset=False):
    DataDirectory = [r'/Users/samzimmerman/Documents/'
                     'Capstone/Data/GDS_deep10_13/']

    AllData = pd.read_csv(
        '/Users/samzimmerman/Documents/Capstone/Data/CombinedData.csv'
    )

    AllData['Size'] = pd.Series(AllData['SN_H']*5/.06, index=AllData.index)

    CombDat = pd.DataFrame(columns=(
        'ID_2', 'Size', 'f_Spheroid', 'f_Disk', 'f_Irr', 'f_PS', 'Rpet_cir_H',
        'C_H', 'A_H', 'S_H', 'G_H', 'M20_H', 'M_H', 'I_H', 'D_H', 'Rpet_cir_J',
        'C_J', 'A_J', 'S_J', 'G_J', 'M20_J', 'M_J', 'I_J', 'D_J', 'Rph', 'Ch',
        'A90h', 'A180h', 'Sh', 'Gh', 'M20h', 'Mh', 'Ih', 'Dh')
    )
    ''' , 'Rpj', 'Cj', 'A90j', 'A180j', 'Sj', 'Gj', 'M20j', 'Mj', 'Ij', 'Dj',
    'Rpv', 'Cv', 'A90v', 'A180v', 'Sv', 'Gv', 'M20v', 'Mv', 'Iv', 'Dv', 'Rpz',
    'Cz', 'A90z', 'A180z', 'Sz', 'Gz', 'M20z', 'Mz', 'Iz', 'Dz'))'''
    ...
    for GN in tqdm(Files):
        ...
        try:
            ...
            HData = ma.masked_array(np.array(hdulist[0].data),
                                    segarray != int(GalName[11:16]))
            # plt.imshow(HData)
            # plt.show()
...
def CompareResults():
    ...
    plt.scatter(Spheroids['Rpet_cir_H'], Spheroids['Rph'], c='b',
                label='Spheroids')
    plt.scatter(Disks['Rpet_cir_H'], Disks['Rph'], c='g', label='Disks')
    plt.scatter(Irregular['Rpet_cir_H'], Irregular['Rph'], c='r',
                label='Irregulars')
    plt.scatter(PointSource['Rpet_cir_H'], PointSource['Rph'], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(Spheroids['C_H'][Spheroids['C_H'] != -99],
                Spheroids['Ch'][Spheroids['C_H'] >= -90], c='b',
                label='Spheroids')
    plt.scatter(Disks['C_H'][Disks['C_H'] >= -90],
                Disks['Ch'][Disks['C_H'] != -99], c='g', label='Disks')
    plt.scatter(Irregular['C_H'][Irregular['C_H'] >= -90],
                Irregular['Ch'][Irregular['C_H'] >= -90], c='r',
                label='Irregulars')
    plt.scatter(PointSource['C_H'][PointSource['C_H'] >= -90],
                PointSource['Ch'][PointSource['C_H'] >= -90], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(Spheroids['A_H'][Spheroids['A_H'] >= -90],
                Spheroids['A180h'][Spheroids['A_H'] >= -90], c='b',
                label='Spheroids')
    plt.scatter(Disks['A_H'][Disks['A_H'] >= -90],
                Disks['A180h'][Disks['A_H'] >= -90], c='g', label='Disks')
    plt.scatter(Irregular['A_H'][Irregular['A_H'] >= -90],
                Irregular['A180h'][Irregular['A_H'] >= -90], c='r',
                label='Irregulars')
    plt.scatter(PointSource['A_H'][PointSource['A_H'] >= -90],
                PointSource['A180h'][PointSource['A_H'] >= -90], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(Spheroids['S_H'][Spheroids['S_H'] >= -90],
                Spheroids['Sh'][Spheroids['S_H'] >= -90], c='b',
                label='Spheroids')
    plt.scatter(Disks['S_H'][Disks['S_H'] >= -90],
                Disks['Sh'][Disks['S_H'] >= -90], c='g', label='Disks')
    plt.scatter(Irregular['S_H'][Irregular['S_H'] >= -90],
                Irregular['Sh'][Irregular['S_H'] >= -90], c='r',
                label='Irregulars')
    plt.scatter(PointSource['S_H'][PointSource['S_H'] >= -90],
                PointSource['Sh'][PointSource['S_H'] >= -90], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(PointSource['G_H'], PointSource['Gh'], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(Spheroids['M20_H'], Spheroids['M20h'], c='b',
                label='Spheroids')
    plt.scatter(Disks['M20_H'], Disks['M20h'], c='g', label='Disks')
    plt.scatter(Irregular['M20_H'], Irregular['M20h'], c='r',
                label='Irregulars')
    plt.scatter(PointSource['M20_H'], PointSource['M20h'], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(PointSource['I_H'], PointSource['Ih'], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(PointSource['M_H'], PointSource['Mh'], c='k',
                label='Point Sources')
    plt.grid()
    ...
    plt.scatter(PointSource['D_H'], PointSource['Dh'], c='k',
                label='Point Sources')
    plt.grid()
...
def Compare2(DataFile):
    ...
    Extent = [np.min(Spheroids['M20_H']), np.max(Spheroids['M20_H']),
              np.min(Spheroids['I_H']), np.max(Spheroids['I_H'])]
    ...
    # plt.scatter(Spheroids['M20_H'], Spheroids['G_H'], c='b',
    #             label='Spheroids')
    # plt.scatter(Disks['M20_H'], Disks['G_H'], c='g', label='Spheroids')
    # plt.scatter(Irregular['M20_H'], Irregular['G_H'], c='r',
    #             label='Spheroids')
    plt.show()
...
def Compare2():
    ...
    Extent = [np.min(Spheroids['M20_H']), np.max(Spheroids['M20_H']),
              np.min(Spheroids['G_H']), np.max(Spheroids['G_H'])]
    ...
    a2.imshow(COLOR, extent=Extent)
    a2.set_title('My stuff')
    # plt.scatter(Spheroids['M20_H'], Spheroids['G_H'], c = 'b', label =
    #             'Spheroids')
    # plt.scatter(Disks['M20_H'], Disks['G_H'], c = 'g', label = 'Spheroids')
    # plt.scatter(Irregular['M20_H'], Irregular['G_H'], c = 'r',
    #             label = 'Spheroids')
...
DataDirectory = [r'/Users/samzimmerman/Documents/'
                 'Capstone/Data/Goods-South-Deep10/']
