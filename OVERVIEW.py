...
AllData = pd.read_csv("/Users/samzimmerman/Documents/Capstone/Data/COSAllGalfit/AllCOSData.csv", index_col=0)
...
for File in tqdm(glob("/Users/samzimmerman/Documents/Capstone/Sam_Jeyhan_Sample/**/*_gf_changed_content.fits")):
    DATAHduList = fits.open(File)
    RA = DATAHduList[0].header["CRVAL1"] - DATAHduList[0].header['CRPIX1']*DATAHduList[0].header['CDELT1']
    DEC = DATAHduList[0].header["CRVAL2"] - DATAHduList[0].header['CRPIX2']*DATAHduList[0].header['CDELT2']
    #print RA, DEC
    ...
    for i in count(1):
        if "COMP_{:d}".format(i) in InfoHDU.header:
            ...
            if InfoHDU.header["COMP_{:d}".format(i)] == 'sersic':
                #print True
                Info.append([Clean(InfoHDU.header["{:d}_MAG_0".format(i)]), Clean(InfoHDU.header["{:d}_XC_0".format(i)]), Clean(InfoHDU.header["{:d}_YC_0".format(i)]), Clean(InfoHDU.header["{:d}_N_0".format(i)]), Clean(InfoHDU.header["{:d}_RE_0".format(i)])])
        else:
            break
    Info.sort()
    if len(Info) > 1:
        Dc = np.sqrt((Info[0][1]-Info[1][1])**2 + (Info[0][2]-Info[1][2])**2)/(min(Info[0][4], Info[1][4]))
        Dn = abs(Info[1][3] - Info[0][3])
        Class = [df_closerow["f_Spheroid"]>0.6,df_closerow["f_Disk"]>0.6,df_closerow["f_Irr"]>0.6, df_closerow["f_any"]>0.5]
        W.writerow([df_closerow.name, File[57:-24], Dc, Dn, C, ''.join(np.where(Class, ['s', 'd', 'i', 'm'], ''))])
        if Class == [1, 1, 1, 1]:
            Class == [0, 0, 0, 1]
